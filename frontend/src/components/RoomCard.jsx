...
import { Users, Bed, Maximize2, Eye, Check, MessageSquare, Sparkles } from 'lucide-react';
import { getRoomImage, HOTEL_IMAGES } from '../data/hotelImages';

export default function RoomCard({ room, nights = 1, onAskAboutRoom }) {
  const [imgSrc, setImgSrc] = useState(getRoomImage(room.room_type_id));
          ...
          <div>
            ...
            {nights > 1 && (
              <span className="text-[11px] font-medium text-amber-800 block mt-0.5">
                Total for {nights} nights: ?{totalPrice?.toLocaleString('en-IN')}
              </span>
            )}
          </div>

          <button
            onClick={() => onAskAboutRoom && onAskAboutRoom(room.room_name)}
            className="px-4 py-2.5 rounded-xl bg-slate-900 hover:bg-amber-600 hover:text-slate-950 active:scale-95 text-white font-medium text-xs transition-all flex items-center gap-1.5 shrink-0 shadow-xs cursor-pointer"
          >
